...
function RoleList() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('http://localhost:6600/getrole');
  ...
  const deleteapi = (role_id) => {
    axios.delete(`http://localhost:6600/deleterole/${role_id}`)
      .then(() => {
        fetchData();
      })
      .catch((error) => {
  ...
  const postapi = (e) => {
    e.preventDefault();
    axios.post("http://localhost:6600/postrole", formData)
      .then(() => {
        fetchData();
  ...
  const updateapi = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:6600/putrole/${editFormData.role_id}`, editFormData)
      .then(() => {
        fetchData();
            ...
            {paginatedData.map((item, index) => (
              ...
              >
                ...
                <StyledTableCell>
                  ...
                  <DeleteIcon sx={{ color: '#1e293b', fontSize: '20px', marginLeft: '20px' }} onClick={() => deleteapi(item.role_id)} />
                </StyledTableCell>
              </StyledTableRow>
            ))}
          </TableBody>
